...
class ViewPagerAdapter (private var title : List<String>, private var description : List<String>, private var images : List<Int>) : RecyclerView.Adapter<ViewPagerAdapter.ViewPageHolder>()  {
    ...
    override fun onCreateViewHolder(
        ...
        binder = DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.view_pager_layout, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewPagerAdapter.ViewPageHolder, position: Int) {
        ...
        binder.imageContainer.setOnClickListener {
            when (position) {
                0 -> Toast.makeText(holder.itemView.context, holder.itemView.context.getString(R.string.swift_and_fast), Toast.LENGTH_SHORT).show()
                1 -> Toast.makeText(holder.itemView.context, holder.itemView.context.getString(R.string.security_assured), Toast.LENGTH_SHORT).show()
                2 -> Toast.makeText(holder.itemView.context, holder.itemView.context.getString(R.string.vaccinated_stuff), Toast.LENGTH_SHORT).show()
            }
        }
    }
